...
namespace PingPong {
	...
	private:
		...
		/// <summary>
		/// Настройка обработчиков событий для проверки ввода
		/// </summary>
		void SetupEventHandlers()
		{
			// Цвета - проверка при изменении текста
			leftColorBox->TextChanged += gcnew EventHandler(this, &Settings::ColorTextBox_TextChanged);
			rightColorBox->TextChanged += gcnew EventHandler(this, &Settings::ColorTextBox_TextChanged);
			ballColorBox->TextChanged += gcnew EventHandler(this, &Settings::ColorTextBox_TextChanged);

			// Скорость - только цифры + проверка диапазона
			ballSpeedBox->KeyPress += gcnew KeyPressEventHandler(this, &Settings::BallSpeed_KeyPress);
			ballSpeedBox->TextChanged += gcnew EventHandler(this, &Settings::BallSpeed_TextChanged);
		}

		/// <summary>
		/// Проверка полей цвета (имя цвета или HEX)
		/// </summary>
		void ColorTextBox_TextChanged(System::Object^ sender, System::EventArgs^ e)
		{
			...
			if (IsValidColor(text) || IsValidHexColor(text))
			{
				try
				{
					Color::FromName(text); // Дополнительная проверка
					colorBox->BackColor = SystemColors::Window;
				}
				catch (...)
				{
		...
		/// <summary>
		/// Проверка стандартных имен цветов
		/// </summary>
		bool IsValidColor(String^ colorName)
		{
			// Основные цвета System::Drawing::Color
		...
		/// <summary>
		/// Проверка HEX формата (#RRGGBB или RRGGBB)
		/// </summary>
		...
		/// <summary>
		/// Блокировка нецифровых символов в поле скорости
		/// </summary>
		void BallSpeed_KeyPress(System::Object^ sender, System::Windows::Forms::KeyPressEventArgs^ e)
		{
			// Разрешаем только цифры 0-9, Backspace и Delete
		...
		/// <summary>
		/// Проверка диапазона скорости (0-100)
		/// </summary>
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->leftColorBox->Text = L"Red";
			// 
			// leftSize
			// 
			this->leftSize->Location = System::Drawing::Point(43, 94);
			this->leftSize->Maximum = 15;
			...
			this->rightColorBox->Text = L"Blue";
			// 
			// ballBox
			// 
			...
			this->ballColorBox->Text = L"Yellow";
			// 
			// ballSpeedBox
			// 
